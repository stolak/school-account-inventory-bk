...
Alternatively, increase your income by ₦${neededIncome.toFixed(
  ...
  async eligibilityAndScoreNew(livedataNew: LiveDataNew) {
    const { purchaseAmount, downPaymentAmount, months, existingLoanRepayment, incomeStability } =
      livedataNew;
    const monthlyIncome = incomeStability.averageIncome;
    const existingMonthlyRepayment = existingLoanRepayment;
    let eligibility;
  ...
  > {
    ...
    const reworkedLatestAccountDetailsData: ReworkedLatestAccountDetailsData = {
      incomeRecurrent: {
        incomeMonths: 5,
      ...
      incomeStability: {
        ...
        monthlyIncomes: [220000, 240000, 250000, 255000, 260000, 275000],
      },
